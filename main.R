...
# keep the units with at least 30 and at most 48 measurements and starting on 10/25/2019, ending on 11/09/2019
unit_count <- planteye_data%>% group_by(unit) %>% 
  summarize(n=n(), mindate=as.Date(min(timestamp)), maxdate=as.Date(max(timestamp)))
...
for(property in properties) {
   # data cleaning
    ...
    lentime <- (maxtime - mintime)/3600 # in Hour
    # auc estimation 
